...
Your processed signals should be named using the conventions used by the baseline system, i.e.,
for Task 1 `<Listener ID>/<Song Name>/<Listener ID>_<Song Name>_<Channel>_<Stem>.flac` and
`<Listener ID>/<Song Name>/<Listener ID>_<Song Name>_remix.wav` as explained in
[Task1 data page](../Data/cc1_data_overview_headphone), and for
