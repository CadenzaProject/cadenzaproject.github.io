...
To obtain the data and baseline code, please see the [download page](../take_part/download) link.

## A. Training, validation and evaluation data

The music dataset is based on the MUSDB18-HQ [[1](#refs)] dataset and a subset of the OlHeaD-HRTF [[2](#refs)] dataset.

### A.1 Training data

The training data needs to be generated by the entrants. We provide:
...
1. Download and install PyClarity as described in [Download data and software](download).
2. Download and extract the **core** data packages as described in [Download data and software](download). 
...
Unlike the training data, the validation data doesn't need to be generates as it is provided as a downloadable package in [Download data and software](download).
The validation data was constructed by randomly taking 2 10-second excepts from each of the 100 tracks of the MUSDB18-HQ train split.
Then, to each excerpt, a random left/rigth HRTFs were applied. This means that 2 excerpts from the same track will have a different HRTF applied.
...
We leave the teams to decide how to use these as part of the training of their systems.
...
:::warning Important 
The correct use of these datasets for supplementing your training data is under the teams' responsibility.
The samples are not containing HRTFs applied to them, and we are not providing manners to apply them. 
However, you can see how the HRTFs are applied to the MUSDB18-HQ dataset in the baseline code.
:::
...
We provide metadata characterising the hearing abilities of listeners, so the audio signals can be individualised. 
The quantification of the listeners' hearing is done with left and right pure tone air-conduction audiograms.
These measure the threshold at which people can hear a pure-tone sound. 
...
Visit [Listener Metadata](data_listener) for more details of the listener characteristic.

### C.2 Gains

We provide metadata indicating the gains use to rebalance the mixture, totalling 1105 possible gains combination:

* 1, 2, or 3 instruments.
* discrete values of [-10, -6, -3, 3, 6, 10] dB LUFS

The same set of gains are used for training, validation and evaluation.
...
### C.7 At hearing aids microphones music
